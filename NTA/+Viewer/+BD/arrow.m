...
for k=firstprop:2:nargin,
  ...
  elseif strncmp(prop,'parent',6),   ax         = val;
  elseif strncmp(prop,'axl',3),      axl         = val;
...
if(~isempty(ax)), tmp = ax; else, tmp = gca; end
if(length(o) == 1)
    ax = tmp;
else
    ax = o * tmp;
...
%	compute lengths
	...
	Dii=find(D==0); 
  if ~isempty(Dii), D=D+(D==0); le(Dii)=zeros(1,length(Dii)); end; %should fix DivideByZero warnings
...
%     end;
        ...
        if ~isempty(zz), zz=zz(mask); end;
		end;
		% plot the patch or line
		xyz = {'XData',xx,'YData',yy,'ZData',zz,'Tag',ArrowTag,'Parent',directParent};
...
% 	set(H,{'UserData'},num2cell(ud,2));
	...
	[H,oldaxlims,errstr] = arrow_clicks(H,ud,x,y,z,ax);
...
% 		ARROW_AXLIMITS = oldaxlims(any(oldaxlims(:,2:7)'~=lims),:);
...
% wait for button down ignoring object ButtonDownFcn's
	...
	if wasInterrupted, errstr=e; end;
